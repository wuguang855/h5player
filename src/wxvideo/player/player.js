...
var Player = function(url, options) {
	...
	if (options.source) {
		...
		this.source = new JWSSource(url, options);
...
Player.prototype.showHide = function(ev) {
	if (document.visibilityState === 'hidden') {
		if(this.options.backFun.audioPlayer){this.options.backFun.audioPlayer.pause();}
		this.unpauseOnShow = this.wantsToPlay;
		this.pause();
	}
	else if (this.unpauseOnShow) {
		if(this.options.backFun.audioPlayer){this.options.backFun.audioPlayer.play();}
		this.play();
	}
};
...
Player.prototype.updateForStaticFile = function() {
	...
	if (notEnoughData && this.source.completed) {
		if (this.loop) {
			...
			if(this.options.backFun.status>0){
				window.clearInterval(this.options.backFun.catonTimer);
				window.clearTimeout(this.options.backFun.beginPlayLoading);
				this.options.backFun.show();
			}
			this.pause();
		}
	}
